...
* Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
*/
public class LovPhongBanAction extends BaseDispatchAction {
	public ActionForward execute(
		...
		if (timKiemCqtForm.getMaCqt() != null
			&& !timKiemCqtForm.getMaCqt().trim().equals("")) {
			criteria.addSearchItem(
				"ma",
				"%" + timKiemCqtForm.getMaCqt().trim().toUpperCase() + "%",
				SearchCriteria.OP_LIKE);
		}
		String donviBiTTKT = request.getParameter("donviBiName");
		if (donviBiTTKT != null && !donviBiTTKT.equals(""))
			timKiemCqtForm.setTenCqt(donviBiTTKT);
		if (timKiemCqtForm.getTenCqt() != null
			&& !timKiemCqtForm.getTenCqt().trim().equals("")) {
			criteria.addSearchItem(
				"ten",
				"%" + timKiemCqtForm.getTenCqt().toUpperCase() + "%",
				SearchCriteria.OP_LIKE);
		}
		ApplicationContext appContext =
			(ApplicationContext) request.getSession().getAttribute(
		...
		if (KtnbUtil.isTongCuc(appContext)) {
			...
			criteria.addSearchItem(
				"ma",
				appContext.getMaCqt().substring(0, 3) + "%",
				SearchCriteria.OP_LIKE);
		}
		criteria.addSearchItem(
			"ma",
		...
		Collection listCQT =
			new BaseHibernateDAO().retrieveObjects(appContext, criteria);

		// Bo sung them co quan thue dang thuc hien vao ket qua tim kiem		
		boolean addCqt = false;
		if (Formater.isNull(timKiemCqtForm.getMaCqt())
			&& Formater.isNull(timKiemCqtForm.getTenCqt()))
			addCqt = true;
		else if (
			Formater.isNull(timKiemCqtForm.getMaCqt())
				&& !Formater.isNull(timKiemCqtForm.getTenCqt())) {
			if (appContext.getTenCqt().indexOf(timKiemCqtForm.getTenCqt())
				>= 0)
				addCqt = true;
		} else if (
			!Formater.isNull(timKiemCqtForm.getMaCqt())
			...
			!Formater.isNull(timKiemCqtForm.getMaCqt())
				&& !Formater.isNull(timKiemCqtForm.getTenCqt())) {
			if (appContext.getMaCqt().indexOf(timKiemCqtForm.getMaCqt()) >= 0
				&& appContext.getTenCqt().indexOf(timKiemCqtForm.getTenCqt())
					>= 0)
				addCqt = true;
		}
		if (addCqt) {
			...
			arrTemp.add(
				new KtnbDmCqt(appContext.getMaCqt(), appContext.getTenCqt()));
	...
	public ActionForward changeCqt(
		ActionMapping mapping,
		ActionForm form,
		HttpServletRequest request,
		HttpServletResponse response) {
