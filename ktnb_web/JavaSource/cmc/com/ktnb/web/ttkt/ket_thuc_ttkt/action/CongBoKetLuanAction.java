...
public class CongBoKetLuanAction extends BaseDispatchAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		ApplicationContext appContext = (ApplicationContext) request
				.getSession().getAttribute(Constants.APP_CONTEXT);
		String method = request.getParameter("method");
		if (cuocTtktId == null) {
			cuocTtktId = congBoKetLuanForm.getIdCuocTtkt();
		}
		TtktKhCuocTtkt cuocTtkt = CuocTtktService.getCuocTtkTrongQd(appContext,
				cuocTtktId);
		...
		if ("save".equals(method)) {
			...
			if (!KtnbUtil.haveRoleNv(appContext)
					|| appContext.getTrangThaiCuocTtkt(cuocTtktId).equals(
							Constants.TT_TTKT_KT))
				request.setAttribute("readOnly", "true");
		}
		try {
			request.setAttribute("ngayRaKetLuanSv", TtktService.getKetLuan(
					appContext, cuocTtktId).getTgianThTuNgayStr());
		} catch (Exception e) {
	...
	private void loadDsTvDoan(String cuocTtktId, HttpServletRequest request) {
		Collection dsThanhVienDoan = TtktService
				.getDanhSachThanhVienDoan(cuocTtktId);
		...
		while (iter.hasNext()) {
			TtktCbChiTietDsTvDoan thanhVienDoan = (TtktCbChiTietDsTvDoan) iter
					.next();
			cbDsThanhVienDoan += "<option value = "
					+ thanhVienDoan.getIdCanBo() + ">";
		...
		request.setAttribute("dsIdThanhVienDoan", dsIdThanhVienDoan.substring(
				0, dsIdThanhVienDoan.length()));
		request.setAttribute("dsChucVuTrongDoan", dsChucVuTrongDoan.substring(
				0, dsChucVuTrongDoan.length()));
		request.setAttribute("chiTietDsTvd", dsThanhVienDoan);
	}

	private void loadDefaulForm(CongBoKetLuanForm form,
			ApplicationContext appContext, String cuocTtktId) throws Exception {
		TtktKtBbCongBoKetLuan congBoQd = TtktCnPhuService.getCongBoKl(
				appContext, cuocTtktId);
		if (congBoQd != null) {
			...
			form.setThoiDiemCongBo(Formater.date2strDateTimeForNV(congBoQd
					.getThoiDiemCongBo()));
			form.setYkienPhatBieu(congBoQd.getYkDaiDienCqTienHanh());
			form.setYkDaiDienCqtBiTtkt(congBoQd.getYkDaiDienCqtBiTtkt());
			form
					.setYkCqqlCapTrenCuaDtuong(congBoQd
							.getYkCqqlCapTrenCuaDtuong());
			form.setThoiDiemKetThuc(Formater.date2strDateTimeForNV(congBoQd
					.getThoiDiemKetThuc()));
			// Dai dien cqql cap tren
			ArrayList list = (ArrayList) congBoQd.getDsTptdCquanTchucCnhan();
			if (!Formater.isNull(list)) {
				TtktCmThanhPhanThamDu[] arrDaiDienCqqlCapTren = new TtktCmThanhPhanThamDu[list
						.size()];
				for (int i = 0; i < list.size(); i++)
					arrDaiDienCqqlCapTren[i] = (TtktCmThanhPhanThamDu) list
							.get(i);
			...
			if (!Formater.isNull(list)) {
				TtktCmThanhPhanThamDu[] arrDaiDienCqtBanHanhQD = new TtktCmThanhPhanThamDu[list
						.size()];
				for (int i = 0; i < list.size(); i++)
					arrDaiDienCqtBanHanhQD[i] = (TtktCmThanhPhanThamDu) list
							.get(i);
			...
			if (!Formater.isNull(list)) {
				TtktCmThanhPhanThamDu[] arrDaiDienDoanTtkt = new TtktCmThanhPhanThamDu[list
						.size()];
				for (int i = 0; i < list.size(); i++)
			...
			if (!Formater.isNull(list)) {
				TtktCmThanhPhanThamDu[] arrDaiDienDonViDuocTtkt = new TtktCmThanhPhanThamDu[list
						.size()];
				for (int i = 0; i < list.size(); i++)
					arrDaiDienDonViDuocTtkt[i] = (TtktCmThanhPhanThamDu) list
							.get(i);
				form.setArrDaiDienDonViDuocTtkt(arrDaiDienDonViDuocTtkt);
			}

		} else {
			form.setDiaDiem(KtnbUtil.getTenTinhByMaCqt(appContext,
					CuocTtktService.getCuocTtkt(appContext, cuocTtktId)
							.getIdDonViBi()));
	...
	private void saveCongBoKL(HttpServletRequest request,
			ApplicationContext appContext, CongBoKetLuanForm form)
			throws Exception {
	...
	private TtktKtBbCongBoKetLuan createCongBoQd(ApplicationContext appContext,
	...
	private void inCongBoKLv3(HttpServletRequest request, ApplicationContext app, CongBoKetLuanForm form, HttpServletResponse reponse, String idCuocTtKt) throws Exception {
		...
		TtktCmThanhPhanThamDu[] arrthanhphanbg1 = form
				.getArrDaiDienCqtBanHanhQD();
		TtktCmThanhPhanThamDu[] arrthanhphanbg4 = form
				.getArrDaiDienCqqlCapTren();
		String fileIn = "";
		String fileOut = "";
		...
		if (arrthanhphanbg1 != null && arrthanhphanbg4 != null) {
			...
			fileOut = request.getRealPath("/docout") + "\\TTNB39_Out"
					+ System.currentTimeMillis() + request.getSession().getId()
					+ ".doc";
		}
		if (arrthanhphanbg1 == null && arrthanhphanbg4 != null) {
			fileIn = request.getRealPath("/docin") + "\\TTNB39_1.doc";
			fileOut = request.getRealPath("/docout") + "\\TTNB39_1_Out"
					+ System.currentTimeMillis() + request.getSession().getId()
					+ ".doc";
		}
		if (arrthanhphanbg1 != null && arrthanhphanbg4 == null) {
			fileIn = request.getRealPath("/docin") + "\\TTNB39_2.doc";
			fileOut = request.getRealPath("/docout") + "\\TTNB39_2_Out"
					+ System.currentTimeMillis() + request.getSession().getId()
					+ ".doc";
		}
		if (arrthanhphanbg1 == null && arrthanhphanbg4 == null) {
			fileIn = request.getRealPath("/docin") + "\\TTNB39_3.doc";
			fileOut = request.getRealPath("/docout") + "\\TTNB39_3_Out"
					+ System.currentTimeMillis() + request.getSession().getId()
					+ ".doc";
		}
		String hinhThuc = (cuocTtkt.getHinhThuc().booleanValue()) ? "Ki\u1EC3m tra"
				: "thanh tra";
		String hinhThuc_in = (cuocTtkt.getHinhThuc().booleanValue()) ? "KT"
				: "TT";
		...
		try {
			...
			if (form.getDiaDiem().equals("")
					|| form.getThoiDiemCongBo().equals("")) {
				word.put("[noi_ra_thong_bao]", ".....");
				word.put("[ngay_thong_bao]",
						"ng\u00E0y.....th\u00E1ng.....n\u0103m.....");
			} else {
				word.put("[noi_ra_thong_bao]", form.getDiaDiem());
				word.put("[ngay_thong_bao]", KtnbUtil.getDay(form
						.getThoiDiemCongBo()));
			}
			word.put("[ttkt]", hinhThuc);

			word.put("[thoi_diem_cong_bo]", KtnbUtil.getHour(form
					.getThoiDiemCongBo()));
			...
			word.put("[ngay_qd]", Formater.getDateForPrint(Formater
					.date2str(cbQd.getNgayRaQuyetDnh())));
			word.put("[thu_truong]", KtnbUtil.getChucVuThuTruongByMaCqt(app
					.getMaCqt()));
			word.put("[ttkt]", hinhThuc);
			word.put("[thu_truong_cqt]", KtnbUtil.getChucVuThuTruongByMaCqt(app
					.getMaCqt()));
			word.put("[ttkt]", hinhThuc);
			...
			if (arrthanhphanbg1 != null) {
				...
				for (int i = 0; i < arrthanhphanbg1.length; i++) {
					tptd.append(KtnbUtil.layoutTableHoTen(arrthanhphanbg1[i]
							.getHoTen()));
					tptd.append(KtnbUtil.layoutTableChucVu(arrthanhphanbg1[i]
							.getChucVu()));
					tptd.append("\n        ");
					word.addCellTable(table, i + 1, 1,
							"- \u00D4ng (b\u00E0\u00A0): "
									+ arrthanhphanbg1[i].getHoTen());
					word.addCellTable(table, i + 1, 2, "- Ch\u1EE9c v\u1EE5: "
							+ arrthanhphanbg1[i].getChucVu());
			...
			if (arrthanhphanbg1 != null) {
				...
				TtktCmThanhPhanThamDu[] arrthanhphanbg2 = form
						.getArrDaiDienDoanTtkt();
				...
				if (arrthanhphanbg2 != null && arrthanhphanbg2.length > 0) {
					for (int i = 0; i < arrthanhphanbg2.length; i++) {
						tptd
								.append(KtnbUtil
										.layoutTableHoTen(arrthanhphanbg2[i]
												.getHoTen()));
						tptd.append(KtnbUtil
								.layoutTableChucVu(arrthanhphanbg2[i]
										.getChucVu()));
						tptd.append("\n        ");
						word.addCellTable(table, i + 1, 1,
								"- \u00D4ng (b\u00E0\u00A0): "
										+ arrthanhphanbg2[i].getHoTen());
						word.addCellTable(table, i + 1, 2,
								"- Ch\u1EE9c v\u1EE5: "
										+ arrthanhphanbg2[i].getChucVu());
			...
			if (arrthanhphanbg1 != null) {
				...
				TtktCmThanhPhanThamDu[] arrthanhphanbg3 = form
						.getArrDaiDienDonViDuocTtkt();
				tptd = new StringBuffer("");
				if ((arrthanhphanbg3 != null && arrthanhphanbg3.length > 0)) {
					for (int i = 0; i < arrthanhphanbg3.length; i++) {
						tptd
								.append(KtnbUtil
										.layoutTableHoTen(arrthanhphanbg3[i]
												.getHoTen()));
						tptd.append(KtnbUtil
								.layoutTableChucVu(arrthanhphanbg3[i]
										.getChucVu()));
						tptd.append("\n        ");
						word.addCellTable(table, i + 1, 1,
								"- \u00D4ng (b\u00E0\u00A0): "
										+ arrthanhphanbg3[i].getHoTen());
						word.addCellTable(table, i + 1, 2,
								"- Ch\u1EE9c v\u1EE5: "
										+ arrthanhphanbg3[i].getChucVu());
			...
			if (arrthanhphanbg4 != null) {
				...
				if (arrthanhphanbg4 != null && arrthanhphanbg4.length > 0) {
					for (int i = 0; i < arrthanhphanbg4.length; i++) {
						tptd
								.append(KtnbUtil
										.layoutTableHoTen(arrthanhphanbg4[i]
												.getHoTen()));
						tptd.append(KtnbUtil
								.layoutTableChucVu(arrthanhphanbg4[i]
										.getChucVu()));
						tptd.append("\n        ");
						word.addCellTable(table, i + 1, 1,
								"- \u00D4ng (b\u00E0\u00A0): "
										+ arrthanhphanbg4[i].getHoTen());
						word.addCellTable(table, i + 1, 2,
								"- Ch\u1EE9c v\u1EE5: "
										+ arrthanhphanbg4[i].getChucVu());
			...
			TtktThBbChitietvatonghop ketLuan = TtktService.getKetLuan(app,
					idCuocTtKt);
			if (ketLuan != null) {
				word.put("[so_kl]", ketLuan.getSoKetLuan());
				word.put("[ngay_ra_kl]", Formater.getDateForPrint(Formater
						.date2str(ketLuan.getTgianThTuNgay())));
			}
			word.put("[thu_truong_cqt]", KtnbUtil.getChucVuThuTruongByMaCqt(app
					.getMaCqt()));
			...
			word.put("[ttkt]", hinhThuc);
			word.put("[thoi_diem_ket_thuc]", KtnbUtil.getHour(form
					.getThoiDiemKetThuc()));
	...
	private void inCongBoKLv4(HttpServletRequest request, ApplicationContext app, CongBoKetLuanForm form, HttpServletResponse reponse, String idCuocTtKt) throws Exception {
		...
		// cbQd.get
		if (arrthanhphanbg1 != null && arrthanhphanbg4 != null) {
			fileIn = request.getRealPath("/docin/v4") + "\\TTNB38.doc";
			fileOut = request.getRealPath("/docout") + "\\TTNB38_Out"
					+ System.currentTimeMillis() + request.getSession().getId()
					+ ".doc";
		}
		if (arrthanhphanbg1 == null && arrthanhphanbg4 != null) {
			fileIn = request.getRealPath("/docin/v4") + "\\TTNB38_1.doc";
			fileOut = request.getRealPath("/docout") + "\\TTNB38_1_Out"
					+ System.currentTimeMillis() + request.getSession().getId()
					+ ".doc";
		}
		if (arrthanhphanbg1 != null && arrthanhphanbg4 == null) {
			fileIn = request.getRealPath("/docin/v4") + "\\TTNB38_2.doc";
			fileOut = request.getRealPath("/docout") + "\\TTNB38_2_Out"
					+ System.currentTimeMillis() + request.getSession().getId()
					+ ".doc";
		}
		if (arrthanhphanbg1 == null && arrthanhphanbg4 == null) {
			fileIn = request.getRealPath("/docin/v4") + "\\TTNB38_3.doc";
			fileOut = request.getRealPath("/docout") + "\\TTNB38_3_Out"
					+ System.currentTimeMillis() + request.getSession().getId()
					+ ".doc";
		}
		// String hinhThuc = (cuocTtkt.getHinhThuc().booleanValue()) ?
		// "Ki\u1EC3m tra" : "thanh tra";
		String hinhThuc = "Ki\u1EC3m tra";
		// String hinhThuc_in = (cuocTtkt.getHinhThuc().booleanValue()) ? "KT" :
		// "TT";
		//String hinhThuc_in = "KT";
		StringBuffer sb = new StringBuffer(hinhThuc);
		sb = new StringBuffer("\u0110o\u00E0n Ki\u1EC3m tra ");
		//sb.append(hinhThuc_in);
		sb.append(" Q\u0110 S\u1ED1 ");
		sb.append(cbQd.getSoQuyetDinh());
...
//			word.put("[ttkt]", hinhThuc.toUpperCase());
			word.put("[ten_truong_doan]", cuocTtkt.getTenTruongDoan());
			word.saveAndClose();
			word.downloadFile(fileOut, "Mau TTNB38", ".doc", reponse);
		} catch (Exception ex) {
			// ex.printStackTrace();
			System.out.println("Download Error: " + ex.getMessage());
		} finally {
