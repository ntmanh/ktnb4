/*
 * Created on Oct 15, 2010
 *
 * To change the template for this generated file go to
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
package cmc.com.ktnb.web.ttkt.dung_chung.form;
import cmc.com.ktnb.entity.TTKT.dung_chung.TtktCmThayDoiThemMoiTvd;
import cmc.com.ktnb.web.BaseActionForm;

/**
 * @author ntdung1
 *
 * To change the template for this generated type comment go to
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class ThayDoiThanhVienDoanForm extends BaseActionForm {
	private String id;
	private String idCuocTtkt;
	private String loaiThayDoi;
	private String soQd;
	private String noiRaQd;
	private String ngayRaQd;
	private String vbanQdinhCnangNvu;
	private String idNguoiDeNghi;
	private String tenNguoiDeNghi;
	private String chucVuNguoiDeNghi;
	private String ngayThoiThamGia;
	private String ngayBanGiao;
	private String ngayBdauCuaTvienMoi;
	private String danhSachThayDoi;
	private String idThanhVienCu;
	private String tenThanhVienCu;
	private String chucVuThanhVienCu;
	private String idThanhVienMoi;
	private String tenThanhVienMoi;
	private String chucVuThanhVienMoi;
	private String donViCongTacTvMoi;
	private TtktCmThayDoiThemMoiTvd[] noiDungThayDoi;
	private TtktCmThayDoiThemMoiTvd[] noiDungBoSung;
	private String dungTieuSao;
	private String idDstvCu;
	private String idDstvMoi;
	private String dvCnCoLquan;

	public String getDvCnCoLquan() {
		return dvCnCoLquan;
	}

	public void setDvCnCoLquan(String dvCnCoLquan) {
		this.dvCnCoLquan = dvCnCoLquan;
	}

	/**
	 * @return
	 */
	public String getChucVuNguoiDeNghi() {
		return chucVuNguoiDeNghi;
	}

	/**
	 * @return
	 */
	public String getChucVuThanhVienCu() {
		return chucVuThanhVienCu;
	}

	/**
	 * @return
	 */
	public String getChucVuThanhVienMoi() {
		return chucVuThanhVienMoi;
	}

	/**
	 * @return
	 */
	public String getDanhSachThayDoi() {
		return danhSachThayDoi;
	}

	/**
	 * @return
	 */
	public String getId() {
		return id;
	}

	/**
	 * @return
	 */
	public String getIdCuocTtkt() {
		return idCuocTtkt;
	}

	/**
	 * @return
	 */
	public String getIdNguoiDeNghi() {
		return idNguoiDeNghi;
	}

	/**
	 * @return
	 */
	public String getIdThanhVienCu() {
		return idThanhVienCu;
	}

	/**
	 * @return
	 */
	public String getIdThanhVienMoi() {
		return idThanhVienMoi;
	}

	/**
	 * @return
	 */
	public String getLoaiThayDoi() {
		return loaiThayDoi;
	}

	/**
	 * @return
	 */
	public String getNgayBanGiao() {
		return ngayBanGiao;
	}

	/**
	 * @return
	 */
	public String getNgayBdauCuaTvienMoi() {
		return ngayBdauCuaTvienMoi;
	}

	/**
	 * @return
	 */
	public String getNgayRaQd() {
		return ngayRaQd;
	}

	/**
	 * @return
	 */
	public String getNgayThoiThamGia() {
		return ngayThoiThamGia;
	}

	/**
	 * @return
	 */
	public String getNoiRaQd() {
		return noiRaQd;
	}

	/**
	 * @return
	 */
	public String getSoQd() {
		return soQd;
	}

	/**
	 * @return
	 */
	public String getTenNguoiDeNghi() {
		return tenNguoiDeNghi;
	}

	/**
	 * @return
	 */
	public String getTenThanhVienCu() {
		return tenThanhVienCu;
	}

	/**
	 * @return
	 */
	public String getTenThanhVienMoi() {
		return tenThanhVienMoi;
	}

	/**
	 * @return
	 */
	public String getVbanQdinhCnangNvu() {
		return vbanQdinhCnangNvu;
	}

	/**
	 * @param string
	 */
	public void setChucVuNguoiDeNghi(String string) {
		chucVuNguoiDeNghi = string;
	}

	/**
	 * @param string
	 */
	public void setChucVuThanhVienCu(String string) {
		chucVuThanhVienCu = string;
	}

	/**
	 * @param string
	 */
	public void setChucVuThanhVienMoi(String string) {
		chucVuThanhVienMoi = string;
	}

	/**
	 * @param string
	 */
	public void setDanhSachThayDoi(String string) {
		danhSachThayDoi = string;
	}

	/**
	 * @param string
	 */
	public void setId(String string) {
		id = string;
	}

	/**
	 * @param string
	 */
	public void setIdCuocTtkt(String string) {
		idCuocTtkt = string;
	}

	/**
	 * @param string
	 */
	public void setIdNguoiDeNghi(String string) {
		idNguoiDeNghi = string;
	}

	/**
	 * @param string
	 */
	public void setIdThanhVienCu(String string) {
		idThanhVienCu = string;
	}

	/**
	 * @param string
	 */
	public void setIdThanhVienMoi(String string) {
		idThanhVienMoi = string;
	}

	/**
	 * @param string
	 */
	public void setLoaiThayDoi(String string) {
		loaiThayDoi = string;
	}

	/**
	 * @param string
	 */
	public void setNgayBanGiao(String string) {
		ngayBanGiao = string;
	}

	/**
	 * @param string
	 */
	public void setNgayBdauCuaTvienMoi(String string) {
		ngayBdauCuaTvienMoi = string;
	}

	/**
	 * @param string
	 */
	public void setNgayRaQd(String string) {
		ngayRaQd = string;
	}

	/**
	 * @param string
	 */
	public void setNgayThoiThamGia(String string) {
		ngayThoiThamGia = string;
	}

	/**
	 * @param string
	 */
	public void setNoiRaQd(String string) {
		noiRaQd = string;
	}

	/**
	 * @param string
	 */
	public void setSoQd(String string) {
		soQd = string;
	}

	/**
	 * @param string
	 */
	public void setTenNguoiDeNghi(String string) {
		tenNguoiDeNghi = string;
	}

	/**
	 * @param string
	 */
	public void setTenThanhVienCu(String string) {
		tenThanhVienCu = string;
	}

	/**
	 * @param string
	 */
	public void setTenThanhVienMoi(String string) {
		tenThanhVienMoi = string;
	}

	/**
	 * @param string
	 */
	public void setVbanQdinhCnangNvu(String string) {
		vbanQdinhCnangNvu = string;
	}

	/**
	 * @return
	 */
	public String getDonViCongTacTvMoi() {
		return donViCongTacTvMoi;
	}

	/**
	 * @param string
	 */
	public void setDonViCongTacTvMoi(String string) {
		donViCongTacTvMoi = string;
	}

	/**
	 * @return
	 */
	public TtktCmThayDoiThemMoiTvd[] getNoiDungThayDoi() {
		return noiDungThayDoi;
	}

	/**
	 * @param tvds
	 */
	public void setNoiDungThayDoi(TtktCmThayDoiThemMoiTvd[] tvds) {
		noiDungThayDoi = tvds;
	}

	/**
	 * @return
	 */
	public TtktCmThayDoiThemMoiTvd[] getNoiDungBoSung() {
		return noiDungBoSung;
	}

	/**
	 * @param tvds
	 */
	public void setNoiDungBoSung(TtktCmThayDoiThemMoiTvd[] tvds) {
		noiDungBoSung = tvds;
	}

	/**
	 * @return
	 */
	public String getDungTieuSao() {
		return dungTieuSao;
	}

	/**
	 * @param string
	 */
	public void setDungTieuSao(String string) {
		dungTieuSao = string;
	}

	/**
	 * @return
	 */
	public String getIdDstvCu() {
		return idDstvCu;
	}

	/**
	 * @return
	 */
	public String getIdDstvMoi() {
		return idDstvMoi;
	}

	/**
	 * @param string
	 */
	public void setIdDstvCu(String string) {
		idDstvCu = string;
	}

	/**
	 * @param string
	 */
	public void setIdDstvMoi(String string) {
		idDstvMoi = string;
	}

}
