...
<html:form action="kntc_khxm.do?method=bb">
	...
	<table width="100%">
		...
		<tr>
			<td style="text-align: right;">Nội dung làm việc <font style="color: red">*</font></td>
			<td colspan="3"><html:textarea property="noiDung" onkeypress="imposeMaxLength(this);" style="width:100%;height: 50px;"></html:textarea></td>
		</tr>
		<tr>
...
function xem(){
	...
	if(anSelected.length>0) {
		document.forms[0].action="kntc_khxm.do?method=xemBblv&action=bbxmkn&id="+document.forms[0].maHoSo.value+"&pId="+pId+"&r="+r;
		document.forms[0].submit();
	}
...
function taomoi(){
	document.forms[0].action="kntc_khxm.do?method=taoMoiBblv&action=bbxmkn&id="+document.forms[0].maHoSo.value;
	document.forms[0].submit();
}
...
function xoa(){
	...
	if(anSelected.length>0) {
		if(confirm ("Bạn chắc chắn muốn xóa bản ghi đang chọn?")){
			document.forms[0].action="kntc_khxm.do?method=xoaBblv&action=bbxmkn&id="+document.forms[0].maHoSo.value+"&pId="+pId;
			document.forms[0].submit();
		}
	}
	else showMessage(chua_chon_ban_ghi);
