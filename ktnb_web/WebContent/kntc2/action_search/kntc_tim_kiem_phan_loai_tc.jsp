...
<html:form action="/kntc_tim_kiem_phan_loai_tc.do?method=show">
<div id="super-tabs">
<ul>
	<li><a href="#super-tab0" onclick="redirect('kntc_tim_kiem_phan_loai_kn.do?method=show&action=phanloaiHSTC');"><span>PHÂN LOẠI ĐỀ XUẤT XỬ LÝ TỐ CÁO</span></a></li>
	<li><a href="#super-tab1" onclick="redirect('kntc_tim_kiem_xac_minh_kn.do?method=show&action=xacMinhToCao');"><span>TIẾN HÀNH XÁC MINH</span></a></li>
	<li><a href="#super-tab2" onclick="redirect('kntc_tim_kiem_ket_thuc_kn.do?method=show&action=ketThucToCao');"><span>KẾT THÚC GIẢI QUYẾT</span></a></li>
</ul>	
	<div id="super-tab0">
...
var act = '<%=request.getParameter("action")%>';
  ...
  var url=encodeURI('kntc_tim_kiem_phan_loai_tc.do?method=search&maHs=%'+$('#data').val()+'%&nnd='+$('#nguoiNopDon').val()+'&cbxl='+$('#canBoXL').val()+'&nuq='+$('#nguoiUyQuyen').val()+'&tSearch='+$('#textSearch').val()+'&tt='+$('#tt').val()+'&loai='+$('#loai').val()+'&sDate='+$('#sDate').val()+'&fDate='+$('#fDate').val()+'&cvsDate='+$('#cvsDate').val()+'&cvfDate='+$('#cvfDate').val()+'&action='+act);
...
function del(){
	if (confirm("Bạn có chắc chắn muốn xóa?")){
		...
		window.location = "kntc_tim_kiem_phan_loai_tc.do?method=del&id="+ma;
	}
}
...
function search(){	
	document.forms[0].action='kntc_tim_kiem_phan_loai_tc.do?method=show';
